...
import me.intronate67.TotalWarfare.LocationUtil;
...
public class SignManager implements Listener {
	...
	public void onSignChange(SignChangeEvent e) {
		if (e.getLine(0).equalsIgnoreCase("[[TotalWarfare]")) {
			int id;
			try { id = Integer.parseInt(e.getLine(1)); }
			catch (Exception ex) { MessageManager.getInstance().severe(e.getPlayer(), "That is not a valid arena number!"); return; }
			
			Arena a = ArenaManager.getInstance().getArena(id);
			if (a == null) {
				MessageManager.getInstance().severe(e.getPlayer(), "That is not a valid arena!");
				return;
			}
	...
	public void onPlayerInteract(PlayerInteractEvent e) {
		...
		if (e.getClickedBlock().getState() instanceof Sign) {
			...
			if (s.getLine(0).equalsIgnoreCase("[TotalWarfare]")) {
